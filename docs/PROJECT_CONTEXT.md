...
- 聚合：有效质押、解质押中、可提取本金、未领取收益、累计收益（地址总览与池详情），以及
  可用余额加池内本金和收益的 Core 总资产；存在未读取池时按下限展示。
...
- 本地数据：收藏地址、收藏池和排序偏好支持按当前 Core 网络导出 JSON，并在同网络内
  校验导入、合并恢复；PoS 池管理另支持只包含池基础配置的独立 JSON 导入导出，不包含
  收藏地址、排序偏好或链上查询数据。
- 池详情：持仓、治理锁定、该池累计收益、增加质押 `inQueue`、解质押 `outQueue`、可解
  质押额度和本金可提取状态组成的可展开生命周期时间线，以及局部重试；生命周期默认
  收起，增加质押锁定只展示 `lockBlock > currentBlock` 的未完成节点。
...
- 累计收益：对每个成功读取的已记录池，使用 `userSummary.claimedInterest` 加上实时
  ...
  池详情复用同一单池公式；该指标不计入总资产，避免当前未领取收益重复计算。
...
- PoS 池专用导入导出只处理当前 Core 网络的池 `id`、名称、合约地址、官网和来源字段；
  文件不包含用户地址或完整本地状态，导入仍通过 `infrastructure/storage/localState.ts`
  的 schema、网络校验和池合并规则。
...
- 2026-09-01 已发布 Core 累计收益：`readPoolPosition` 读取用户已领取收益，domain 聚合
  与 Core 地址总览、池详情展示共用同一公式；代码提交 `2c33313`，发布记录提交
  `e81ae9f`，Pages 工作流 `33481178004` 成功。当前基线为 116 项单测、8 项 E2E、类型、
  Lint、格式检查和生产构建全部通过。
