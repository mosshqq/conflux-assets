...
- 地址总览的“预计下次可质押时间”由 `domain/portfolio` 计算：在可用余额和全部池持仓
  ...
  `bigint` 向上取整为秒。有效持仓 APY 不可用或为零时不得给出时间。
- 地址总览的“预计每日收益”同样由 `domain/portfolio` 计算：全部池持仓读取完成后，将
  每池 `activeVotes` 对应本金乘以 `poolAPY()` 基点并汇总，再除以 `10000 × 365`；
  全程使用 `bigint`，任一有效持仓池 APY 不可用时不得展示部分估算。
...
都按上述流程单独重新读取钱包余额、准备交易并校验费用，然后逐笔请求 Fluent 确认和提交；
所有已提交交易完成确认后再统一等待回执并刷新成功项 Query。Fluent 的 `cfx_sendTransaction`
请求同时包含用户确认和提交，不能先取得签名再延后广播；某笔准备失败或被拒绝时停止后续
未提交交易，已提交交易仍继续等待回执，未发送的池保留给用户稍后重试。
...
- 复投时间估算只使用合约 APY 近 7 天年化快照作匀速外推；不读取或猜测未来链上收益，
  并且始终预留 1 CFX 基础费用以符合质押 MAX 规则。
- 预计每日收益只使用有效质押与合约 APY 近 7 天年化快照，按 365 天折算并向下截断到
  Drip；解质押中和可提取本金不计入收益本金。
